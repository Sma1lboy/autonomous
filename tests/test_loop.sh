#!/usr/bin/env bash
# Tests for scripts/loop.py — standalone launcher.
# Since loop.py ends with `exec ... claude`, we can only test:
# - argument parsing and env var handling
# - persona.py invocation
# - prompt construction
...
LOOP="$SCRIPT_DIR/../scripts/loop.py"
...
PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" "test direction" 2>/dev/null || true
assert_eq "$([ -f "$MOCK_DIR/captured.txt" ] && echo 'yes' || echo 'no')" "yes" \
  "loop.py invoked claude"
...
OUTPUT=$(PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" "build REST API" 2>&1 || true)
...
OUTPUT=$(AUTONOMOUS_DIRECTION="fix all bugs" PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" 2>&1 || true)
...
OUTPUT=$(PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" 2>&1 || true)
...
OUTPUT=$(MAX_ITERATIONS=25 PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" 2>&1 || true)
...
PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" "test" 2>/dev/null || true
...
assert_eq "$([ -f "$REPO_ROOT/OWNER.md" ] && echo 'yes' || echo 'no')" "yes" \
  "persona.py auto-generates OWNER.md (global)"
ARGS=$(cat "$MOCK_DIR/args.txt" 2>/dev/null || echo "")
...
ERR=$(PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "/nonexistent/path" 2>&1 || true)
...
ERR=$(PATH="$EMPTY_DIR:/usr/bin:/bin" python3 "$LOOP" "$T" 2>&1 || true)
...
ERR=$(MAX_ITERATIONS="abc" PATH="$MOCK_DIR:$PATH" python3 "$LOOP" "$T" 2>&1 || true)
...
python3 "$LOOP" --help >/dev/null 2>&1
...
EMPTY_DIR=$(new_mock_dir)
ERR=$(PATH="$EMPTY_DIR:/usr/bin:/bin" python3 "$LOOP" "$(new_git_project)" 2>&1 || true)
