#!/usr/bin/env bash
# Tests for scripts/conductor-state.py — state management for the multi-sprint conductor.
...
T=$(new_tmp)
SESSION_ID=$(python3 "$CONDUCTOR" init "$T" "build feature X" 10)
...
echo ""
echo "2. Read state"
STATE=$(python3 "$CONDUCTOR" read "$T")
...
echo "3. Sprint-start"
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "build API" 5 > /dev/null
NUM=$(python3 "$CONDUCTOR" sprint-start "$T" "scaffold Express project")
...
# Second sprint
NUM2=$(python3 "$CONDUCTOR" sprint-start "$T" "add CRUD routes")
...
echo "4. Sprint-end"
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "build tests" 5 > /dev/null
python3 "$CONDUCTOR" sprint-start "$T" "write unit tests" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Added 10 unit tests" '["abc1234"]' "false")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "build feature" 10 > /dev/null

# Sprint 1: complete with commits, direction_complete=true
python3 "$CONDUCTOR" sprint-start "$T" "build core" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "Built core" '["a1"]' "true" > /dev/null

# Sprint 2: complete with commits, direction_complete=true (2nd consecutive)
python3 "$CONDUCTOR" sprint-start "$T" "add tests" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Tests done" '["b2"]' "true")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "long mission" 4 > /dev/null
# max_directed = 70% of 4 = 2 (rounded down, min 1) => actually int(4*0.7) = 2

# Sprint 1
python3 "$CONDUCTOR" sprint-start "$T" "step 1" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "Done step 1" '["c1"]' "false" > /dev/null

# Sprint 2 (should hit max_directed=2)
python3 "$CONDUCTOR" sprint-start "$T" "step 2" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Done step 2" '["c2"]' "false")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "explore stuff" 10 > /dev/null

# Sprint 1: has commits
python3 "$CONDUCTOR" sprint-start "$T" "initial" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "Built initial" '["d1"]' "false" > /dev/null

# Sprint 2: zero commits
python3 "$CONDUCTOR" sprint-start "$T" "refine" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "Nothing to do" '[]' "false" > /dev/null

# Sprint 3: zero commits (2nd consecutive)
python3 "$CONDUCTOR" sprint-start "$T" "more refine" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Still nothing" '[]' "false")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "careful build" 10 > /dev/null

# Sprint 1: direction_complete=true
python3 "$CONDUCTOR" sprint-start "$T" "build" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Built it" '["e1"]' "true")
...
python3 "$CONDUCTOR" sprint-start "$T" "fix bug" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "complete" "Fixed" '["e2"]' "false")
...
for dim in test_coverage error_handling security code_quality documentation architecture performance dx; do
  python3 "$CONDUCTOR" explore-score "$T" "$dim" "8" > /dev/null
done
# Set security low
python3 "$CONDUCTOR" explore-score "$T" "security" "3" > /dev/null
...
STATE=$(python3 "$CONDUCTOR" read "$T" 2>/dev/null)
assert_eq "$STATE" "{}" "read returns empty on missing state"

PHASE=$(python3 "$CONDUCTOR" phase "$T" 2>/dev/null)
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "big project" 10 > /dev/null

for i in 1 2 3; do
  python3 "$CONDUCTOR" sprint-start "$T" "task $i" > /dev/null
  python3 "$CONDUCTOR" sprint-end "$T" "complete" "Done task $i" "[\"commit$i\"]" "false" > /dev/null
done
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "atomic test" 5 > /dev/null
...
python3 "$CONDUCTOR" sprint-start "$T" "task 1" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "Done" '["x"]' "false" > /dev/null
TMP_COUNT=$(find "$T/.autonomous" -name "*.tmp.*" 2>/dev/null | wc -l | tr -d ' ')
...
echo "15. PID lock"
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "lock test" 5 > /dev/null
...
ERR=$(python3 "$CONDUCTOR" init "$T" "second conductor" 5 2>&1 || true)
...
SESSION=$(python3 "$CONDUCTOR" init "$T" "fresh start" 5 2>/dev/null)
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "score test" 5 > /dev/null
RESULT=$(python3 "$CONDUCTOR" explore-score "$T" "security" "6.5")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "phase test" 5 > /dev/null
PHASE=$(python3 "$CONDUCTOR" phase "$T")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "defaults" 10 > /dev/null
python3 "$CONDUCTOR" sprint-start "$T" "task" > /dev/null
PHASE=$(python3 "$CONDUCTOR" sprint-end "$T" "blocked" "Couldn't proceed")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "counter test" 10 > /dev/null

# direction_complete=true
python3 "$CONDUCTOR" sprint-start "$T" "s1" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "done" '["a"]' "true" > /dev/null
...
python3 "$CONDUCTOR" sprint-start "$T" "s2" > /dev/null
python3 "$CONDUCTOR" sprint-end "$T" "complete" "partial" '["b"]' "false" > /dev/null
...
ERR=$(python3 "$CONDUCTOR" foobar "$T" 2>&1 || true)
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "calc test" 20 > /dev/null
MAX_DIR=$(python3 -c "
import json
d = json.load(open('$T/.autonomous/conductor-state.json'))
...
python3 "$CONDUCTOR" init "$T" "scan test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "coverage test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "sec test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "sec issues test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "quality test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "doc test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "arch test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "dx test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "clamp test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "integration test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "empty test" 10 > /dev/null
...
T=$(new_tmp)
mkdir -p "$T"
ERR=$(python3 "$SCANNER" "$T" "$CONDUCTOR" 2>&1 || true)
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "validate test" 10 > /dev/null

ERR=$(python3 "$CONDUCTOR" explore-score "$T" "bogus_dim" "5" 2>&1 || true)
assert_contains "$ERR" "unknown dimension" "invalid dimension rejected"

ERR=$(python3 "$CONDUCTOR" explore-score "$T" "security" "notanumber" 2>&1 || true)
assert_contains "$ERR" "must be numeric" "non-numeric score rejected"

# Valid should succeed
RESULT=$(python3 "$CONDUCTOR" explore-score "$T" "security" "7.5" 2>&1)
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "trap test" 10 > /dev/null
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "clamp edge test" 10 > /dev/null
...
mkdir -p "$TRICKY"
SESSION_ID=$(python3 "$CONDUCTOR" init "$TRICKY" "test mission" 5 2>&1)
...
# Read state back
STATE=$(python3 "$CONDUCTOR" read "$TRICKY" 2>&1)
...
SPRINT=$(python3 "$CONDUCTOR" sprint-start "$TRICKY" "test direction" 2>&1)
assert_eq "$SPRINT" "1" "sprint-start works with single-quote path"

PHASE=$(python3 "$CONDUCTOR" sprint-end "$TRICKY" "complete" "done" "[]" "false" 2>&1)
...
T=$(new_tmp)
SESSION_ID=$(python3 "$CONDUCTOR" init "$T" "build 'feature' with \"quotes\" & \$vars" 5 2>&1)
assert_contains "$SESSION_ID" "conductor-" "init with special char mission"
STATE=$(python3 "$CONDUCTOR" read "$T" 2>&1)
MISSION=$(python3 -c "import json,sys; print(json.loads(sys.argv[1]).get('mission',''))" "$STATE")
...
T=$(new_tmp)
python3 "$CONDUCTOR" init "$T" "test" 5 > /dev/null
SPRINT=$(python3 "$CONDUCTOR" sprint-start "$T" "fix 'bug' in module" 2>&1)
...
echo ""
echo "42. --help flag"
HELP=$(python3 "$CONDUCTOR" --help 2>&1)
...
python3 "$CONDUCTOR" init "$T" "fresh mission" 5 >/dev/null
...
python3 "$CONDUCTOR" init "$T" "new session" 5 >/dev/null
...
python3 "$CONDUCTOR" init "$T" "clean slate" 5 >/dev/null
for i in 1 2 3 4 5; do
